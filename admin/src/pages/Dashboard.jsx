...
export default function Dashboard() {
  ...
  const cards = [
    { label: d.totalNews,   value: counts?.posts,   sub: `${counts?.published} ${d.published} · ${counts?.drafts} ${d.drafts}`, color: 'navy',    to: '/posts',   icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" /></svg> },
    { label: d.gallery,     value: counts?.gallery, sub: d.photosUploaded,  color: 'blue',    to: '/gallery', icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg> },
    { label: d.events,      value: counts?.events,  sub: `${counts?.upcoming} ${d.upcoming}`, color: 'emerald', to: '/events', icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg> },
    { label: d.staff,       value: counts?.staff,   sub: d.activeMembers,   color: 'amber',   to: '/staff',   icon: <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg> },
  ]
  ...
  const quickActions = [
    { label: d.addPost,      to: '/posts/new',    color: 'bg-navy-900 hover:bg-navy-800 text-white' },
    { label: d.uploadImages, to: '/gallery',      color: 'bg-blue-600 hover:bg-blue-700 text-white' },
    { label: d.createEvent,  to: '/events/new',   color: 'bg-emerald-600 hover:bg-emerald-700 text-white' },
    { label: d.editSite,     to: '/homepage',     color: 'bg-crimson-600 hover:bg-crimson-700 text-white' },
  ]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
        {quickActions.map(a => (
          <Link key={a.label} to={a.to} className={`${a.color} rounded-xl px-4 py-3 text-sm font-semibold text-center transition-colors`}>
            {a.label}
          </Link>
        ))}
      </div>
