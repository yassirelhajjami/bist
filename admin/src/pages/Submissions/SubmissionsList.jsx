...
function SubmissionCard({ item, onMarkRead, onDelete }) {
  ...
  return (
    <div className={`card p-5 transition-all ${!item.is_read ? 'border-l-4 border-navy-500' : ''}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {!item.is_read && <span className="badge badge-green text-xs">{s.newBadge}</span>}
            </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button onClick={() => setExpanded(x => !x)} className="btn btn-ghost btn-sm text-slate-500 hover:text-navy-700">
            <svg className={`w-4 h-4 transition-transform ${expanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
            </svg>
          </button>
          {!item.is_read && (
            <button onClick={() => onMarkRead(item.id)} title={s.markRead}
          ...
          <button onClick={() => onDelete(item.id)} className="btn btn-ghost btn-icon btn-sm text-slate-400 hover:text-red-600">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function SubmissionsList() {
  ...
  const load = useCallback(async () => {
    ...
    if (readFilter !== '') q = q.eq('is_read', readFilter === 'true')
    const { data } = await q
    setItems(data || [])
    setLoading(false)
    ...
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {items.map(item => (
            <SubmissionCard key={item.id} item={item} onMarkRead={markRead} onDelete={setDeleteId} />
          ))}
        </div>
      )}
